...
type AppConfig struct {
	Env string; Port string; BaseURL string; FrontendURL string
	LaundryOpenHour int; LaundryCloseHour int; SaunaOpenHour int; SaunaCloseHour int
	LaundryBookAheadWeeks int
}

type DatabaseConfig struct {
	URL string
	Host string; Port string; Name string; User string; Password string; SSLMode string
	MaxOpenConns int; MaxIdleConns int; ConnMaxLifetime time.Duration
}
...
type RedisConfig struct { Addr string; Password string; DB int }
type JWTConfig struct { AccessSecret string; RefreshSecret string; AccessTTL time.Duration; RefreshTTL time.Duration }
type EmailConfig struct { Provider string; APIKey string; FromEmail string; FromName string }
type BankIDConfig struct { Provider string; MockDelay int }
...
func Load() (*Config, error) {
	...
	return &Config{
		...
		Database: DatabaseConfig{
			...
			Name: getEnv("DB_NAME", "hoas_booking"), User: getEnv("DB_USER", "postgres"),
			Password: getEnv("DB_PASSWORD", ""), SSLMode: getEnv("DB_SSLMODE", "disable"),
			MaxOpenConns: getEnvInt("DB_MAX_OPEN_CONNS", 25), MaxIdleConns: getEnvInt("DB_MAX_IDLE_CONNS", 5),
...
func requireEnv(key string) string {
	v := os.Getenv(key)
	if v == "" { panic(fmt.Sprintf("required env var %q not set", key)) }
	return v
}
